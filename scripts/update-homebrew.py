#!/usr/bin/env python3
"""Create or update a Homebrew formula for fabbro."""
import sys
import os
...
base = f"https://github.com/charly-vibes/fabbro/releases/download/{tag}"
...
class Fabbro < Formula
  ...
  on_macos do
    on_arm do
      url "{base}/fabbro_{version}_darwin_arm64.tar.gz"
      sha256 "{shas['darwin_arm64']}"
    end
    on_intel do
      url "{base}/fabbro_{version}_darwin_amd64.tar.gz"
  ...
  on_linux do
    on_arm do
      if Hardware::CPU.is_64_bit?
        url "{base}/fabbro_{version}_linux_arm64.tar.gz"
        sha256 "{shas['linux_arm64']}"
      end
    end
    on_intel do
      url "{base}/fabbro_{version}_linux_amd64.tar.gz"
