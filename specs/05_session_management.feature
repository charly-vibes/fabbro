Feature: Session Management
  ...
  #
  # Note: All scenarios in this spec are now @implemented.
  ...
  Scenario: Listing all sessions
    ...
    When I run the command `fabbro session list`
  ...
  Scenario: Listing sessions in JSON format
    ...
    When I run the command `fabbro session list --json`
  ...
  Scenario: Showing session details
    ...
    When I run the command `fabbro session show review-123`
  ...
  Scenario: Showing session with annotation breakdown
    Given a session exists with:
      ...
      | delete   | 1     |
      | question | 2     |
    When I run the command `fabbro session show <session-id>`
  ...
  Scenario: Showing non-existent session
    ...
    When I run the command `fabbro session show missing`
  ...
  Scenario: Resuming an interrupted review
    ...
    When I run the command `fabbro session resume review-123`
  ...
  Scenario: Resuming in editor mode
    ...
    When I run the command `fabbro session resume review-123 --editor`
  ...
  Scenario: Resuming non-existent session
    Given no session "missing" exists
    When I run the command `fabbro session resume missing`
    Then an error message should indicate the session was not found
    And the command should exit with code 1
  ...
  Scenario: Deleting a session
    Given a session "review-123" exists
    When I run the command `fabbro session delete review-123`
  ...
  Scenario: Deleting a session with --force
    Given a session "review-123" exists
    When I run the command `fabbro session delete review-123 --force`
  ...
  Scenario: Deleting non-existent session
    Given no session "missing" exists
    When I run the command `fabbro session delete missing`
    Then an error message should indicate the session was not found
    And the command should exit with code 1
  ...
  Scenario: Cleaning sessions older than threshold
    Given sessions exist with various ages:
      ...
      | old-002     | 14 days |
      | recent-001  | 2 days  |
    When I run the command `fabbro session clean --older-than 7d`
  ...
  Scenario: Dry-run cleaning
    ...
    When I run the command `fabbro session clean --older-than 7d --dry-run`
  ...
  Scenario: Exporting session as standalone file
    Given a session "review-123" exists
    When I run the command `fabbro session export review-123 --output review.fem`
  ...
  Scenario: Exporting session to stdout
    Given a session "review-123" exists
    When I run the command `fabbro session export review-123`
  ...
  Scenario: Partial session ID matching
    ...
    When I run the command `fabbro session show abc1`
  ...
  Scenario: Ambiguous partial session ID
    Given sessions exist:
      ...
      | review-abc123  |
      | review-abc456  |
    When I run the command `fabbro session show abc`
